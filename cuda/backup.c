...
static void try_shift_head(Block*);
...
static void lockEventList(int, int);
static void unlockEventList(int);
...
//void saveEventList(const char* file_name) {

	//	FILE * file = fopen (file_name, "w");
	//
	//	Block* block = glbEventList.blocks;
	//	while(block != NULL) {
	//		for(unsigned int i = 0; i < block->size; ++i) {
	//			Event* e = &block->events[i];
	//			fprintf(file, "Tid: %d, Kind: %d, Val: %d \n", e->tid, e->kind, e->value);
	//		}
	//
	//		block = block->next;
	//	}
...
// get the next index to insert a new event
static inline unsigned long nextEventIndex() {
	return INC(&glbEventList.next_index);
}
...
static inline bool isEventListLocked() {
	static ReentrantLock* lock = &glbEventList.lock;
	return islockedReentrant(lock);
}
...
static void addEvent(EventKind kind, int tid, long value, int instr) {
	...
	//IF_V_HEAD(printf("Adding new event.\n"));
	...
	start:
		...
		//IF_V_HEAD(printf("Adding event: <%d(%d) by %d> to %d\n", kind, value, tid, index));
	
		//-------------------------------
		// if we are the last inserter, then try to shift head
		if(index == BLOCK_SIZE-1) {
			IF_V_HEAD(printf("Tid %d shifts\n", tid));
			IF_V_HEAD(fflush(stdout));
			try_shift_head(block);
		}
...
static inline void try_shift_head(Block* block) {
	...
	else IF_V_HEAD(printf("WTF -> prev head = %p, new head = %p\n", &block, &head));
}
